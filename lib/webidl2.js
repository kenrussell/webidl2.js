...
(() => {
  ...
  function parse(tokens, opt) {
    ...
    function consume_with_ws(type, value, store) {
      const ws = all_ws();
    ...
    function integer_type() {
      const ids = [];
      const prefix = consume_with_ws(ID, "unsigned", ids);
      const base = consume_with_ws(ID, "short", ids) || consume_with_ws(ID, "long", ids);
    ...
    function float_type() {
      const ids = [];
      const prefix = consume_with_ws(ID, "unrestricted", ids)
      const base = consume_with_ws(ID, "float", ids) || consume_with_ws(ID, "double", ids);
      if (prefix && !base) {
        return error("Failed to parse integer type");
      }
      if (base) {
    ...
    function primitive_type() {
      ...
      const token =
        consume_with_ws(ID, "boolean") ||
        consume_with_ws(ID, "byte") ||
        consume_with_ws(ID, "octet");
      if (token) {
        return {
          tokens: [token],
          type: token.value
    ...
    function const_(store) {
      const ws = all_ws(store, "pea");
      if (!consume(ID, "const")) return;
      const ret = { type: "const", nullable: null, ws };
      let typ = primitive_type();
      if (!typ) {
        typ = consume_with_ws(ID) || error("No type for const");
      }
      ret.idlType = Object.assign({ type: "const-type" }, EMPTY_IDLTYPE, { idlType: typ.type, ws: typ.ws });
      ret.nullable = consume_with_ws(OTHER, "?");
      ret.name = consume_with_ws(ID) || error("No name for const");
      ret.equalSign = consume_with_ws(OTHER, "=") || error("No value assignment for const");
      const cnt = const_value();
      if (cnt) ret.value = cnt;
      else error("No value for const");
      ret.termination = consume_with_ws(OTHER, ";") || error("Unterminated const");
      return ret;
    }
