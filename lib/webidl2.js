...
(() => {
  ...
  function parse(tokens, opt) {
    ...
    function const_(store) {
      const trivia = { 
        definition: all_ws(store, "pea").value
      };
      ...
      trivia.equalSignTrivia = (consume_with_trivia(OTHER, "=") || error("No value assignment for const")).trivia;
      const cnt = const_value();
      if (cnt) ret.value = cnt;
      else error("No value for const");
      trivia.terminationTrivia = (consume_with_trivia(OTHER, ";") || error("Unterminated const")).ws;
      return ret;
    }
