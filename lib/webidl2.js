...
(() => {
  ...
  function parse(tokens, opt) {
    ...
    function type_suffix(obj) {
      while (true) {
        const nullable = consume_existence(OTHER, "?");
        if (nullable) {
    ...
    function type_with_extended_attributes(typeName) {
      ...
      if (extAttrs && ret) ret.extAttrs = extAttrs.items;
      return ret;
    }

    function argument(store) {
      const ret = { optional: false, variadic: false };
      ret.extAttrs = extended_attrs(store).items;
      all_ws(store, "pea");
    ...
    function extended_attrs(store) {
      ...
      const open = consume_with_trivia(OTHER, "[");
      if (!open) return eas;
      eas.trivia = {
        open: open.trivia,
        dividers: []
      };
      eas.items[0] = simple_extended_attr(store) || error("Extended attribute with not content");
      while (consume_with_trivia(OTHER, ",", eas.trivia.dividers)) {
        eas.items.push(simple_extended_attr(store) || error("Trailing comma in extended attribute"));
      }
      const close = consume(OTHER, "]") || error("No end of extended attribute");
    ...
    function callback(store) {
      const base = consume_with_trivia(ID, "callback");
      if (!base) return;
      const tok = consume_with_trivia(ID, "interface");
      if (tok) {
        // TODO: add callback trivia
        return interface_rest(false, store, "callback interface");
      }
      const trivia = {
        base: base.trivia
      };
      const name = consume_with_trivia(ID) || error("No name for callback");
      trivia.name = name.trivia;
      const ret = current = { 
        type: "callback",
        name: sanitize_name(name.value, "callback"),
        trivia
      };
      const assignment = consume_with_trivia(OTHER, "=") || error("No assignment in callback");
      trivia.assignment = assignment.trivia;
      all_ws();
      ret.idlType = return_type();
      const open = consume_with_trivia(OTHER, "(") || error("No arguments in callback");
      trivia.open = open.trivia;
      ret["arguments"] = argument_list(store) || [];
      const close = consume_with_trivia(OTHER, ")") || error("Unterminated callback");
      trivia.close = close.trivia;
      const termination = consume_with_trivia(OTHER, ";") || error("Unterminated callback");
      trivia.termination = termination.trivia;
      return ret;
    }
    ...
    function enum_(store) {
      const base = consume_with_trivia(ID, "enum");
      if (!base) return;
      const trivia = {
        base: base.trivia
      }
      const name = consume_with_trivia(ID) || error("No name for enum");
      trivia.name = name.trivia;
      const vals = [];
      const ret = current = {
        type: "enum",
      ...
      const open = consume_with_trivia(OTHER, "{") || error("No curly for enum");
      trivia.open = open.trivia;
      let saw_comma = false;
      while (true) {
        const close = consume_with_trivia(OTHER, "}");
        if (close) {
          trivia.close = close.trivia;
          const termination = consume_with_trivia(OTHER, ";") || error("No semicolon after enum");
          trivia.termination = termination.trivia;
          return ret;
        }
    ...
    function typedef(store) {
      const base = consume_with_trivia(ID, "typedef");
      if (!base) return;
      const ret = {
        type: "typedef",
        trivia: {
      ...
      const name = consume_with_trivia(ID) || error("No name in typedef");
      ret.trivia.name = name.trivia;
      ret.name = sanitize_name(name.value, "typedef");
      current = ret;
      const termination = consume_with_trivia(OTHER, ";") || error("Unterminated typedef");
      ret.trivia.termination = termination.trivia;
      return ret;
    }

    function implements_(store) {
      const target = consume_with_trivia(ID);
      if (!target) return;
      const w = all_ws();
      if (consume(ID, "implements")) {
        ...
        const imp = consume_with_trivia(ID) || error("Incomplete implements statement");
        ret.trivia.implements = imp.trivia;
        ret.implements = imp.value;
        const termination = consume_with_trivia(OTHER, ";") || error("No terminating ; for implements statement");
        ret.trivia.termination = termination.trivia;
        return ret;
      } else {
        // rollback
    ...
    function includes(store) {
      ...
      if (consume(ID, "includes")) {
        ...
        const inc = consume_with_trivia(ID) || error("Incomplete includes statement");
        ret.trivia.includes = inc.trivia;
        ret.includes = inc.value;
        all_ws();
        const termination = consume_with_trivia(OTHER, ";") || error("No terminating ; for includes statement");
        ret.trivia.termination = termination.trivia;
        return ret;
      } else {
        // rollback
    ...
    function definitions(store) {
      ...
      while (true) {
        ...
        if (!def) {
          if (ea.items.length) error("Stray extended attributes");
          break;
        }
        def.extAttrs = ea.items;
        defs.push(def);
      }
      return defs;
    }
